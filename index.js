...
app.post('/upload', (req, res) => {
  ...
  const watermarkFile = req.files.watermark; // New: For watermark file
  const selectedFont = req.body.font || 'Arial-Bold';
  const outputFileName = req.body.outputFileName || 'output.mp4';
  const userEmail = req.body.email;
  const watermarkPosition = req.body.watermarkPosition || 'main_w-overlay_w-10:10'; // New: For watermark position
  ...
  if (watermarkFile) {
    watermarkPath = path.join(__dirname, `/uploads/watermark_${uniqueId}.${watermarkFile.name.split('.').pop()}`);
    watermarkFile.mv(watermarkPath, (err) => {
      if (err) {
        console.error(`Error: ${err.message}`);
        return res.status(500).send('Error occurred while uploading the watermark.');
      }
      // Move the video and subtitles files only after the watermark file has been uploaded successfully
      videoFile.mv(videoPath, (err) => {
        ...
        subtitlesFile.mv(subtitlesPath, (err) => {
          ...
          const ffmpegCommand = `ffmpeg -i "${videoPath}" ${watermarkFilter} -vf "subtitles=${subtitlesPath}:force_style='FontName=${fullFontPath}'" "${outputPath}"`;
