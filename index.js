...
app.post('/upload', isAuthenticated, (req, res) => {
  ...
  const ffmpegCommand = `ffmpeg -i "${videoPath}" -i "${logoPath}" -filter_complex "[1][0]scale2ref=w=iw/5:h=ow/mdar[logo][video];[video][logo]overlay=W-w-10:10,subtitles=${subtitlesPath}:force_style='FontName=${selectedFontFile},Bold=1,FontSize=22',scale=${resolution}" ${depth} -c:v libx264 -crf 31 -preset medium -r 23.976 -b:v 1500k -b:a 128k "${outputPath}"`;
...
const processVideoWithoutLogo = () => {
  ...
  const ffmpegCommand = `ffmpeg -i "${videoPath}" -vf "subtitles=${subtitlesPath}:force_style='FontName=${selectedFontFile},Bold=1,FontSize=22',scale=${resolution}" ${depth} "${outputPath}"`;
